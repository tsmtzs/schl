...
\heading{Tests for \textit{schl} package}

\begin{enumerate}
\item Mathematical functions in greek:
  $$\tan_1^3 f(x)\quad \cot x \quad \sin \varphi \quad \cos t$$
  $$\gcd (1,2)\quad \lcm(3,4)$$
\item Set $x\to x_0$ under $\lim$:  $\limdisplay{x\to x_0}f(x)$
\item Blank space\\
...
\item Environment \texttt{exercise} for typesetting exercises.
  \begin{exercise}
...
\item Environment \texttt{multichoice} is for multiple choice questions:
  ...
  \item this is an remarkably long choice 3
  \end{multichoice}
...
\item Logo of the  exams
  ...
  \authorityiii{ΔΙΕYΘΥΝΣΗ Β/ΘΜΙΑΣ ΕΚΠΑΙΔΕΥΣΗΣ ΚΑΛΑΜΑΤΑΣ}
